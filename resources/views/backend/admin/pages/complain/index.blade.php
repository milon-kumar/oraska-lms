        ...
        <div class="row bg-secondary">
            <div class="col-12" style="position: sticky;width: 100%;">
                <div class="page-title-box">
                    ...
                    <a href="{{ route('admin.complain.create') }}" class="btn btn-primary mb-2"><i class="mdi mdi-plus-circle"></i>Create Issue</a>
                </div>
            </div>
        </div>
        <div class="row mt-5">
                    ...
                    <div class="card border border-secondary">
                        <div class="card-body">
                            <a href="" class="float-end btn btn-danger" onclick="event.preventDefault();document.getElementById('deleteForm{{$complain->id ??''}}').submit();">
                                <form action="{{ route('admin.complain.destroy',$complain->id ?? '') }}" class="d-none" id="deleteForm{{$complain->id ??''}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                </form>
